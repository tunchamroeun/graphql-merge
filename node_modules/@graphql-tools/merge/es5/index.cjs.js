...
function mergeResolvers(resolversDefinitions, options) {
    ...
    if (resolversFactories.length) {
        result = (function () {
            ...
            var resultsOfFactories = resolversFactories.map(function (factory) { return factory.apply(void 0, tslib.__spread(args)); });
...
function addDescription(cb) {
    return function (node, _key, _parent, path, ancestors) {
        ...
        var key = tslib.__spread(keys, [parent.name.value]).join('.');
        var items = [];
        if (commentsRegistry[key]) {
            items.push.apply(items, tslib.__spread(commentsRegistry[key]));
        }
        return join(tslib.__spread(items.map(printComment), [node.description, cb(node)]), '\n');
    };
}
...
function mergeDirectives(d1, d2, config) {
    ...
    var result = deduplicateDirectives(tslib.__spread(asNext));
...
function mergeDirective(node, existingNode) {
    if (existingNode) {
        ...
        return tslib.__assign(tslib.__assign({}, node), { locations: tslib.__spread(existingNode.locations, node.locations.filter(function (name) { return !nameAlreadyExists(name, existingNode.locations); })) });
    }
    return node;
}
...
function mergeEnumValues(first, second, config) {
    ...
    var result = tslib.__spread(enumValueMap.values());
    if (config && config.sort) {
        result.sort(utils.compareNodes);
    }
    return result;
...
function mergeNamedTypeArray(first, second, config) {
    var result = tslib.__spread(second, first.filter(function (d) { return !alreadyExists(second, d); }));
    if (config && config.sort) {
        result.sort(utils.compareNodes);
    }
    return result;
...
function mergeGraphQLTypes(types, config) {
    ...
    var allNodes = types
        ...
        return tslib.__spread(defs, newDef);
    }, []);
    ...
    return tslib.__spread(Object.values(mergedNodes), [graphql.parse(schemaDefinition).definitions[0]]);
}
...
function mergeSchemas(config) {
    ...
    extractedResolvers.push.apply(extractedResolvers, tslib.__spread(ensureResolvers(config)));
            ...
            switch (_b.label) {
                case 0: return [4 /*yield*/, Promise.all([
                        ...
                        Promise.all(config.schemas.map(function (schema) { return tslib.__awaiter(_this, void 0, void 0, function () { return tslib.__generator(this, function (_a) {
                            ...
                            return mergeResolvers(tslib.__spread(extractedResolvers, ensureResolvers(config)), config);
                        }),
                        Promise.all(config.schemas.map(function (schema) { return tslib.__awaiter(_this, void 0, void 0, function () { return tslib.__generator(this, function (_a) {
...
function mergeTypes(_a) {
    ...
    return mergeTypeDefs(tslib.__spread(schemas, (typeDefs ? utils.asArray(typeDefs) : [])), config);
}
